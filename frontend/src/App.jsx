...
import Inbox from './components/Inbox'; // NEW
import DirectChat from './components/DirectChat'; // ENSURE IMPORTED
...
function App() {
  ...
  useEffect(() => {
    ...
    callBackend('get_all_communities').then(all => { if(Array.isArray(all)) setJoinedCommunities(all); });
    callBackend('get_user', [currentUserId]).then(data => { if(data && data.id) setCurrentUser(data); });
  }, [currentUserId, activeTab]);

  const handleNavigate = (tab) => { setReturnPath(null); setActiveTab(tab); };

  if (!currentUserId) return <LoginPage onLogin={(id) => setCurrentUserId(id)} />;
  ...
  return (
    <div className="flex min-h-screen bg-void-black bg-[url('/bg.jpg')] bg-cover bg-center text-white font-montserrat">
      ...
      <Sidebar 
        activeTab={activeTab} setActiveTab={handleNavigate} joinedCommunities={joinedCommunities}
        onCommunityClick={(id) => handleNavigate(`comm_${id}`)} currentUserId={currentUserId} currentUser={currentUser}
      />
