...
const Sidebar = ({ activeTab, setActiveTab, joinedCommunities = [], onCommunityClick, currentUserId }) => {
  return (
    <div className="w-20 md:w-64 bg-void-black/90 border-r border-white/10 flex flex-col h-screen fixed left-0 top-0 z-50 backdrop-blur-xl">
      ...
      <div className="h-20 flex items-center justify-center border-b border-white/10">
        <h1 className="hidden md:block font-orbitron text-xl text-cyan-supernova drop-shadow-glow tracking-widest cursor-pointer" onClick={() => setActiveTab('home')}>
          NOVA
        </h1>
      ...
      <nav className="flex-1 overflow-y-auto py-4 space-y-2">
        ...
        >
          <span className="text-xl">📡</span>
          <span className="ml-4 font-montserrat font-bold hidden md:block">Find Signals</span>
        </button>
        ...
        {joinedCommunities.map(c => (
          ...
          >
            <div className="w-8 h-8 rounded-full bg-deep-void flex items-center justify-center text-xs font-bold border border-white/20">
      ...
      <div className="p-4 border-t border-white/10">
        ...
        >
          <div className="w-10 h-10 rounded-full bg-cyan-supernova/20 border border-cyan-supernova overflow-hidden flex items-center justify-center text-cyan-supernova font-bold">
             {currentUserId}
          </div>
